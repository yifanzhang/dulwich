...
class ObjectStoreTests(object):
    ...
    def test_iter_tree_contents_include_trees(self):
        ...
        expected = [
          TreeEntry('', 0o40000, tree_id),
          TreeEntry('a', 0o100644, blob_a.id),
          TreeEntry('ad', 0o40000, tree_ad.id),
          TreeEntry('ad/b', 0o100644, blob_b.id),
          TreeEntry('ad/bd', 0o40000, tree_bd.id),
...
class ObjectStoreGraphWalkerTests(TestCase):
    ...
    def test_descends(self):
        ...
        self.assertEquals("a", next(gw))
        self.assertEquals("b", next(gw))
    ...
    def test_only_once(self):
        ...
        self.assertEquals("a", next(gw))
        self.assertEquals("c", next(gw))
        gw.ack("a")
        self.assertEquals("b", next(gw))
        self.assertEquals("d", next(gw))
        self.assertIs(None, next(gw))
